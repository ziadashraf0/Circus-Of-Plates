...
public class newShape implements Shape {
    ...
    public newShape(PlateShape p) {
        this(p.getX(), p.getY(), true, true, p.getColor(), p.getPosition());

    }

    public newShape(int posX, int posY, boolean left, boolean top, Color color, PlatePos position) {
        this.x = posX;
        this.y = posY;
        this.left = left;
        this.top = top;
        ...
        for (int i = 0; i < spriteImages.length; i++) {
           ...
           //  double theta = j * Math.PI / (2 * spriteImages.length);
            //double theta1 = j * Math.PI / (2 * spriteImages.length+50);
            ...
            g2.drawOval(x1 + 2, y1 + 2, x2 - 4, y2 - 30);
            g2.setColor(color);
            g2.fillOval(x1 + 1, y1 + 1, x2 - 8, y2 - 35);
            g2.dispose();
        }
    }
